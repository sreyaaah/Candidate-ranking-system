...
def generate_training_data():
    ...
    embed_model = SentenceTransformer(config.DENSE_MODEL_NAME, device=device)
    embed_model.max_seq_length = config.MAX_SEQ_LENGTH
    
    # INT8 Quantization for BGE CPU speedup (Fixes Weakness 4)
    # Disabled on macOS due to lack of supported QEngine backend for quantized::linear_prepack
    pass
        
    query_embs = {}
    print("Generating query embeddings...")
    for q in queries:
        print(f"Encoding query {q['id']}...")
        query_embs[q["id"]] = embed_model.encode(
            "Represent this sentence for searching relevant passages: " + q["text"][:1200],
            normalize_embeddings=True
        )
        print(f"Encoded query {q['id']} successfully.")
        
    # Free memory (Fixes Windows Paging File OOM error 1455)
    print("Freeing BGE model...")
    del embed_model
    gc.collect()
    print("Embedding model successfully freed from RAM.")
    
    # Import FAISS and retrieval logic after embedding generation to avoid macOS Apple Silicon conflicts
    import faiss
    from search_hybrid import get_hybrid_scores
        
    ce_device = "cpu"
    print(f"Using device: {ce_device} for CrossEncoder")
    ce_model = CrossEncoder(config.CROSS_ENCODER_MODEL_NAME, max_length=512, device=ce_device)
