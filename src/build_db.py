...
def process_candidates():
    ...
    with open(JSONL_PATH, 'r', encoding='utf-8') as f:
        for line in tqdm(f, total=total):
            ...
            batch.append((candidate_id, full_text, skills_lower, summary, career_text, edu_text, cert_text))
            
            if len(batch) >= batch_size:
                cursor.executemany("INSERT INTO candidates (id, full_text, skills, summary, experience, education, certifications) VALUES (?, ?, ?, ?, ?, ?, ?)", batch)
                batch = []
